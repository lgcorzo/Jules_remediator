    ...
    async fn verify_resource(&self, resource: &ClusterResource) -> Result<bool> {
        ...
        match resource.kind.as_str() {
            "Pod" => {
                ...
                if let Some(pod) = pods.get(&resource.name).await.ok() {
                    let phase_ok = pod.status.as_ref().and_then(|status| status.phase.as_deref())
                        .is_some_and(|p| p == "Running" || p == "Succeeded");
                    
                    let ready_ok = pod.status.as_ref().and_then(|status| status.conditions.as_ref())

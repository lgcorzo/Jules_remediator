...
impl StartupMaster {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            if state.boot_storm_detected && matches!(state.phase, StartupPhase::Controlled | StartupPhase::Initial | StartupPhase::InProcess) {
    ...
    async fn lockdown(&self) -> Result<()> {
        ...
        let resources = self.remediator.get_tier_resources(DependencyTier::Applications).await?;
    ...
    async fn orchestrate(&self, state: ClusterStartupState) -> Result<()> {
        ...
        println!("[StartupMaster] Phase 4: Releasing applications (Tier 3) in batches of {}...", state.batch_size);
        let tier3 = self.remediator.get_tier_resources(DependencyTier::Applications).await?;
        
        for chunk in tier3.chunks(state.batch_size) {
            for res in chunk {
                self.remediator.resume_resource(res).await?;
            }
            println!("[StartupMaster] Batch released. Waiting {}s for stabilization...", state.release_interval_secs);

...
graph TD
    ...
    C -->|Start Run| D[MLflow]
    C -->|Query Resource| E[K8s API]
    C -->|Context + Resource| F[Jules SDK]
...
### FixProposal
The output from the Jules AI.
- `code_change`: The suggested YAML or CLI patch.
- `risk_score`: (Low, Medium, High).
- `confidence`: (0.0 to 1.0).

### RemediationOutcome
The result of an execution attempt, capturing `success` status and `latency_ms`.

...
3.  **Classification**: The LLM/System classifies the error type (`Transient`, `Permanent`, `Unknown`).
...
7.  **Closure**: The result is logged to MLflow via the `Tracker` port, and the run is marked as `Success` or `Failed`.
...
| `latency_ms` | Time from alert reception to fix application. | < 5 mins |
